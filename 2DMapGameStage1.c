...
int main(void) {
    ...
    int pieces=0,points=0,row=0,col=0;
    char command = 0;
    ...
    for (int i=0; i<pieces; i++){
        ...
        if ((points >-10 &&points <10) && !(row==7&&col ==0)){
            if (points>0) { map[row][col].occupier = HEALING_TYPE; map[row][col].points = points;}
            else if (!points){ map[row][col].occupier = MONSTER_TYPE; map[row][col].points = points;}
            else {map[row][col].occupier = BOULDER_TYPE; map[row][col].points = -points;}
        } else continue;
    } 
